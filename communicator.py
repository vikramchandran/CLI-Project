...
class Communicator(Session):
    ...
    def removefile(self, which, args, username):
        ...
        if which == 'remove':
            filename = '/' + args['rkey']
            s3res.Object('bucket name', 'Hashed/' + username + filename).delete()
    ...
    def upload(self, which, args, username):
        if which == 'upload':
            ...
            if args['replace']:
                try:
                    self.s3res.Object('bucket name', 'Hashed/' + username + filename).load()
                    self.s3res.Object('bucket name',
                                 'Hashed/' + username + filename).delete()
                    self.runscan(args['file'])
                    self.s3client.upload_file(args['file'], 'bucket name',
    ...
    def interact(self, which, args, username):
        ...
        if which == 'dirupload':
            filename = self.dirupload(which, args, 'bucket name',
                                        'Hashed/' + username)
        if which == 'upload':
